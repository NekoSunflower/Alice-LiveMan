...
public class BroadcastServerController {
    ...
    @RequestMapping("/editServer.json")
    public ActionResult editServer(@RequestBody ServerInfo serverInfo) {
        AccountInfo account = (AccountInfo) session.getAttribute("account");
        if (!account.isAdmin()) {
            return ActionResult.getErrorResult("权限不足");
        }
        ...
        for (ServerInfo _server : liveManSetting.getServers()) {
            if (_server.equals(serverInfo)) {
                server = _server;
                server.setAddress(serverInfo.getAddress());
                server.setUsername(serverInfo.getUsername());
                server.setPassword(serverInfo.getPassword());
                server.setPerformance(serverInfo.getPerformance());
